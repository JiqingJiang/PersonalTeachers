...
> **定位说明**：本项目是一个面向小规模用户（~500人）的轻量级 MVP，采用 SQLite 单文件数据库、单机部署，适合个人学习和小团队试用。如需支撑更大规模，可平滑迁移至 PostgreSQL + Redis + 容器化部署。
...
- Python 3.10+
- Node.js 18+（前端构建）
- 至少一个 AI 模型 API Key（DeepSeek / 智谱GLM / 自定义厂商）
- 一个 SMTP 邮箱（推荐 QQ 邮箱，用于发送验证码）
...
# 配置环境变量
cp .env.example .env
# 编辑 .env，必填：SECRET_KEY、ADMIN_EMAIL/PASSWORD、SMTP 配置
```
...
后端启动后访问 http://localhost:8000 ，数据库和种子数据自动初始化。

### 3. 配置 AI 模型（必须）

启动后登录管理后台 http://localhost:8000/admin/ ，在 **AI 模型管理** 中：

1. 点击预设模型（DeepSeek / 智谱GLM）
2. 填入对应的 API Key
3. 启用模型

> DeepSeek API Key：https://platform.deepseek.com/api_keys
> 智谱GLM API Key：https://open.bigmodel.cn/usercenter/apikeys

### 4. 配置推送邮箱（必须，否则无法收到每日推送）

在管理后台 **邮箱池管理** 中添加发件邮箱：

- 填入 SMTP 信息（QQ 邮箱推荐，需生成授权码）
- 设置每日发送上限
...
项目提供完整的部署配置（nginx + systemd），参考 `deploy/` 目录。
