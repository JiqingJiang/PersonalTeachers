...
class DailyQuoteScheduler:
    ...
    def start(self, delivery_time: str = "08:00"):
        ...
        if self.scheduler.running:
            logger.info("Scheduler already running, stopping first...")
    ...
    def restart(self, delivery_time: str = "08:00"):
        """
        重启调度器（使用新的推送时间）

        Args:
            delivery_time: 新的推送时间，格式 "HH:MM"
        """
        logger.info(f"Restarting scheduler with new time: {delivery_time}")
        self.start(delivery_time)
